...
CSV  — вертикальный формат (несколько строк):
  ...
  · DOCUMENTS_GOST — названия через запятую, например:
    "ГОСТ Р 72274-2025,ГОСТ 1811-2019,СП 32.13330.2018"
  · Результаты анализов: скалярные RESULTS_PH, RESULTS_IRON, ...
    (TEST_RESULTS реконструируется импортером автоматически)
